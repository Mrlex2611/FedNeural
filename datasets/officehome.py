...
class OneDomainDataset(data.Dataset):
    def __init__(self, data_name, root, train=True, transform=None, target_transform=None):
        ...
        self.transform = transform
        self.target_transform = target_transform
        self.domain_map = {'Art': 0, 'Clipart': 1, 'Product': 2, 'RealWorld': 3}
...
class FedLeaOfficeHome(FederatedDataset):
    NAME = 'fl_officehome'
    SETTING = 'domain_skew'
    DOMAINS_LIST = ['Art', 'Clipart', 'Product', 'RealWorld']
    percent_dict = {'Art': 1, 'Clipart': 0.33, 'Product': 0.25, 'RealWorld': 0.5}

    N_SAMPLES_PER_Class = None
    N_CLASS = 65
    OFFICEHOME_TRANSFORM = transforms.Compose([
    ...
    def get_data_loaders(self, selected_domain_list=[]):
        ...
        nor_transform = self.OFFICEHOME_TRANSFORM
        ...
        for _, domain in enumerate(self.DOMAINS_LIST):
            ...
            domain_dataset = ImageFolder(data_path() + domain + '/', nor_transform)
            domain_dataset.data_name = domain
    ...
    def get_backbone(parti_num, names_list):
        ...
        if names_list == None:
            for j in range(parti_num):
                ...
                nets_list.append(autoencoder(FedLeaOfficeHome.N_CLASS))
        else:
            for j in range(parti_num):
